          ...
          <div className="grid grid-cols-3 gap-2">
            {paymentMethods.map((e, index) => {
              return (
                <div
                  key={index}
                  className={`flex flex-col items-center justify-center w-27 h-17 p-2 rounded-sm ${getPaymentColor(
                    e
                  )} ${
                    selectedMethod == e
...
function getPaymentColor(pm: PaymentMethod): string {
  if (pm.name.toLocaleLowerCase().includes("bsi")) {
    return "bg-white";
  }

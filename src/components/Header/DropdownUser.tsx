  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="badge badge-outline badge-primary mx-4 my-2 text-sm font-medium text-black dark:text-white">
          {user?.role?.toLocaleUpperCase()}
        </span>
        {ranger != null ? (
          <span className="px-4 text-sm font-medium text-black dark:text-white">
            {ranger?.divisi?.name} - {ranger?.divisi?.regional}
          </span>
        ) : (
          <div></div>
        )}
        <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
          ...
          <li>
            <button
              ...
              onClick={() => {
                setDialogContent(
                  <FormAccount toggleDialog={toggleFormDialog} />
                );
                toggleFormDialog();
              }}
            >
        ...
        <button
          ...
          className="flex items-center gap-3.5 py-4 px-6 text-sm font-medium duration-300 ease-in-out text-danger hover:text-primary lg:text-base"
        >
          <svg
            className="fill-current"
            width="22"
      ...
      {/* <!-- Dropdown End --> */}
      <Dialog ref={dialogRef} toggleDialog={toggleFormDialog}>
        {dialogContent}
      </Dialog>
    </div>
  );
